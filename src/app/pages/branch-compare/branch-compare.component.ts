...
export class BranchCompareComponent implements OnInit {
  ...
  multiQueue = signal<
    import('../../core/services/branch-compare-state.service').BranchComparePreFill[]
  >([]);
  multiIndex = signal(0);

  get hasNext() {
    return this.multiIndex() < this.multiQueue().length - 1;
  }
  get hasPrev() {
  ...
  filteredCommits = computed(() => {
    ...
    return res.commits.filter((cwt) => {
      const msg = cwt.commit.message.trim();
      if (excludeMerges && BranchCompareComponent.MERGE_PATTERNS.some((p) => p.test(msg)))
        return false;
  ...
  ngOnInit(): void {
    ...
    this.fromSearch$.pipe(debounceTime(300), distinctUntilChanged()).subscribe(() => {
      this.loadRefsForSide('from', false);
    });

    this.toSearch$.pipe(debounceTime(300), distinctUntilChanged()).subscribe(() => {
   ...
   */
  private _applyPreFill(
    queue: import('../../core/services/branch-compare-state.service').BranchComparePreFill[],
  ) {
        ...
        if (side === 'from') {
          ...
          if (isLoadMore) {
            ...
            if (this.toType() === 'branch' && !this.toRef()) {
              ...
              const preferred =
                newItems.find((o: any) => o.name === defaultBranch) ??
  ...
  analyzeGap(): void {
    ...
    this.bitbucket
      .analyzeGap(r, f, t, p)
      .pipe(finalize(() => this.analyzingGap.set(false)))
      .subscribe({
  ...
  get totalTickets() {
    return this.result()?.ticketSummary.total || 0;
  }
  get donePct() {
    ...
    try {
      ...
      navigator.clipboard
        ...
        .then(() => {
          ...
          this.notify.success(
            `Copied ${rows.length} tickets — paste into Confluence as a rich table!`,
          );
