...
import {
  ...
  PredictionMode,
  BacktestResult,
} from '../../core/services/crypto-ai.service';
...
export class CryptoDashboardComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  loadData() {
    ...
    this.binance
      .getHistoricalCandles(this.selectedSymbol(), this.selectedInterval(), 200)
      .subscribe((candles) => {
  ...
  runPrediction() {
    ...
    if (mode === 'gemini') {
      obs = this.cryptoAi.predictWithGemini(
        this.candles,
        this.selectedSymbol(),
  ...
  runBacktest() {
    ...
    this.cryptoAi
      .runBacktest(this.selectedSymbol(), this.selectedInterval(), mode)
      .subscribe((result) => {
  ...
  trainModel() {
    ...
    this.cryptoAi.trainModel(this.selectedSymbol(), this.selectedInterval()).subscribe((result) => {
  ...
  getSourceHint(): string {
    switch (this.aiSource()) {
      case 'gemini':
        return 'Uses Gemini 2.5 Flash Lite via your API key';
      case 'rule':
        return 'Rule-based with 15+ indicators & confluence scoring';
      case 'ml':
        return 'XGBoost ML with Optuna tuning (requires Python server)';
      default:
        return '';
    }
  }
