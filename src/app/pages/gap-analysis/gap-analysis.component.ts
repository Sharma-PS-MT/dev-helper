...
import {
  GapAnalysisEngineService,
  ServiceGapResult,
  GapEntry,
  GapStatus,
} from '../../core/services/gap-analysis-engine.service';
import {
  GapAnalysisStateService,
  GapServiceState,
} from '../../core/services/gap-analysis-state.service';
...
export class GapAnalysisComponent implements OnInit {
  ...
  private deriveGroupedStatus(
    ...
    const hasSameMessage = Array.from(forwardMessages).some((message) =>
      reverseMessages.has(message),
    );
  ...
  private filterRows(rows: DisplayGapRow[]): DisplayGapRow[] {
    return rows.filter((row) => {
      const statusMatch =
        this.filterGapStatuses.length === 0 || this.filterGapStatuses.includes(row.status);
      const searchText = [
        row.ticketId,
      ...
      return statusMatch && this.containsText(searchText, this.filterTextSearch);
    });
  }
  ...
  copyToConfluence() {
    ...
    const summaryRows = this.selectedProjects
      .map(
        (p) => `
      <tr>
    ...
    for (const result of this.results) {
      ...
      const gapRows = displayGaps
        .map((gap, index) => {
          ...
          const ticketCell =
            resolvedUrl && ticketKey !== 'No Ticket'
    ...
    try {
      const item = new ClipboardItem({
        ...
        'text/plain': new Blob(
          ['Please paste into a rich-text editor (like Confluence) to see the tables.'],
          { type: 'text/plain' },
        ),
      });
      navigator.clipboard
