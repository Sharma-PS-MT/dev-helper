    ...
    <div class="version-dialog">
      ...
      <div class="dialog-body">
        ...
        <div class="ref-type-toggle">
          ...
          <button mat-button [class.active]="refType() === 'tag'" (click)="onTypeChange('tag')">
          ...
          <mat-action-list *ngIf="options().length > 0">
            ...
            >
              <mat-icon matListItemIcon>{{
                refType() === 'branch' ? 'account_tree' : 'sell'
              }}</mat-icon>
              <span matListItemTitle class="ref-title">{{ opt.displayId || opt.name }}</span>
              <span matListItemLine class="commit-hash">{{
                (opt.latestCommit || opt.latestChangeset)?.substring(0, 8)
              }}</span>
            </button>
          </mat-action-list>
...
export class VersionSelectDialogComponent implements OnInit {
  ...
  constructor(
    ...
    this.searchSubject
      .pipe(takeUntilDestroyed(), debounceTime(400), distinctUntilChanged())
      .subscribe(() => {
        this.start.set(0);
